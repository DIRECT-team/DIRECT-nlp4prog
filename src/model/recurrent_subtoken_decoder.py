...
from utils.process_ast import AbstractSyntaxTree
...
class RecurrentSubtokenDecoder(Decoder):
    def __init__(self, variable_encoding_size: int, hidden_size: int, dropout: float, tie_embed: bool, input_feed: bool, vocab: Vocab):
        ...
        # bert_config = BertConfig(vocab_size=self.vocab_size, max_position_embeddings=512, num_hidden_layers=6, hidden_size=256, num_attention_heads=4, is_decoder=True)
        ...
        #     self.bert_model.state_dict()[key].copy_(new_state_dict[key])
    ...
    def forward(self, src_ast_encoding, prediction_target):
        ...
        # attention_mask = src_ast_encoding['seq_encoding_result']['code_token_mask']
        # encoder_hidden_states = src_ast_encoding['seq_encoding_result']['code_token_encoding']
        # shifted_outputs = torch.cat([torch.ones(variable_tgt_name_id.shape[0], 1).long().to(variable_tgt_name_id.device), variable_tgt_name_id], axis=1)
        # shifted_outputs = shifted_outputs[:, :-1]
        # query_vecs = self.bert_model(input_ids=shifted_outputs, encoder_hidden_states=encoder_hidden_states, encoder_attention_mask=attention_mask)[0]
