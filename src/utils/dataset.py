...
from utils.process_ast import AbstractSyntaxTree, SyntaxNode
...
class Batcher(object):
    ...
    def to_batched_prediction_target(self, examples: List[Example]):
        ...
        source_word2id          = self.vocab.source_tokens.word2id
        target_word2id          = self.vocab.target.word2id
        all_subtoken_word2id    = self.vocab.all_subtokens.word2id
        target_id2word          = self.vocab.target.id2word

        src_with_true_var_names = []
        src_with_true_var_mask = []
        variable_name_maps = []
        ...
        for e_id, example in enumerate(examples):
            ast = example.ast
            var_name_map = example.variable_name_map
            variable_name_maps.append(var_name_map)
            _var_node_ids = []
            _tgt_name_ids = []
            example_with_true_var_names = []
            example_with_true_var_mask = []
            variable_ptr = 0
            for i, sub_token in enumerate(example.sub_tokens):
                if sub_token.startswith('@@') and sub_token.endswith('@@'):
                    old_var_name = sub_token[2: -2]
                    if old_var_name in example.ast.variables:  # sometimes there are strings like `@@@@`
                        target_subtoken_ids = example.variable_name_subtoken_map[old_var_name]
                        target_subtokens = [target_id2word[t] for t in target_subtoken_ids]
                        unique_subtoken_ids = [all_subtoken_word2id[t] for t in target_subtokens]

                        if unique_subtoken_ids[0] == all_subtoken_word2id['<IDENTITY>']:
                            unique_subtoken_ids[0] = all_subtoken_word2id[sub_token]
                            assert len(unique_subtoken_ids) == 2

                        example_with_true_var_names += unique_subtoken_ids
                        example_with_true_var_mask  += [1] * len(unique_subtoken_ids)
                    else:
                        if sub_token not in all_subtoken_word2id:
                            sub_token = '<unk>'
                        example_with_true_var_names.append(all_subtoken_word2id[sub_token])
                        example_with_true_var_mask.append(0)
                else:
                    if sub_token not in all_subtoken_word2id:
                        sub_token = '<unk>'
                    example_with_true_var_names.append(all_subtoken_word2id[sub_token])
            ...
            src_with_true_var_names.append(example_with_true_var_names)
            src_with_true_var_mask. append(example_with_true_var_mask)
        ...
        max_len     = max([len(l) for l in src_with_true_var_names])
        temp        = np.zeros((len(src_with_true_var_names), max_len), dtype=np.int32)
        temp_mask   = np.zeros((len(src_with_true_var_names), max_len), dtype=np.float32)

        for i, l in enumerate(src_with_true_var_names):
            temp[i, 0:len(l)] = l
            temp_mask[i, 0:len(l)] = src_with_true_var_mask[i]

        src_with_true_var_names = torch.LongTensor(temp)
        src_with_true_var_mask  = torch.FloatTensor(temp_mask)

        source_to_target_maps = []

        for var_map in variable_name_subtoken_maps:
            new_map = {}
            for v in var_map.keys():
                v_mod = '@@'+v+'@@'
                if v_mod not in source_word2id: v_mod = '<unk>'
                new_map[source_word2id[v_mod]] = [all_subtoken_word2id[target_id2word[c]] for c in var_map[v]]

                if var_map[v][0] == target_word2id['<IDENTITY>']:
                    assert len(var_map[v]) == 2
                    new_map[source_word2id[v_mod]][0] = all_subtoken_word2id[v_mod]

            source_to_target_maps.append(new_map)
        ...
        return dict(variable_tgt_name_id                    = variable_tgt_name_id,
                    ...
                    src_with_true_var_names                 = src_with_true_var_names,
                    src_with_true_var_mask                  = src_with_true_var_mask,
                    source_to_target_maps                   = source_to_target_maps,
                    variable_name_subtoken_maps             = variable_name_subtoken_maps)


def get_json_iterator_from_tar_file(file_paths, shuffle=False, progress=False, group_by=None, buffer=True, truncate=-1, seed=42) -> Iterable:
...
def json_line_reader(file_path, queue, worker_num, shuffle, progress, group_by=None, buffer=True, truncate=False, seed=42):
    limit = 30000 if truncate else -1
    for json_str in get_json_iterator_from_tar_file(file_path, shuffle, progress, group_by=group_by, buffer=buffer, truncate=limit, seed=seed):
        queue.put(json_str)
...
def example_to_batch(json_queue, batched_examples_queue, batch_size, train, config, worker_manager_lock, return_examples=False, return_prediction_target=True, max_seq_len=-1, test_only=False):
    ...
    def _generate_batches():
        ...
        for i, example in enumerate(buffer):
            if max_seq_len != -1 and example.source_seq_length > max_seq_len: continue

            if test_only and example.test_meta['function_body_in_train'] is True: continue
...
class Dataset(object):
    ...
    def batch_iterator(self, batch_size: int, config: Dict,
                       ...
                       num_readers=3,
                       num_batchers=3,
                       progress=True, train=False, single_batcher=False, truncate=False, shuffle=True, seed=42, test_only=False) -> Iterable[Union[Batch, Dict[str, torch.Tensor]]]:
        if progress:
        ...
        if single_batcher:
            ...
            return it_func(self._batch_iterator(batch_size, config, num_readers, num_batchers, train, return_examples, return_prediction_target, max_seq_len, truncate, shuffle, seed, test_only))

    def _batch_iterator(self, batch_size: int, config: Dict, num_readers, num_batchers, train=False, return_examples=False, return_prediction_target=None, max_seq_len=-1, truncate=False, shuffle=True, seed=42, test_only=False) -> Iterable[Batch]:
        ...
        json_loader = multiprocessing.Process(target=json_line_reader,
                                              args=(self.file_paths, json_enc_queue, num_readers,
                                                    shuffle, False, None, True, truncate, seed))
        json_loader.daemon = True
        example_generators = []
        ...
        for i in range(num_readers):
            p = multiprocessing.Process(target=example_to_batch,
                                        args=(json_enc_queue, worker_result_queue, batch_size, train, config, worker_manager_lock, return_examples, return_prediction_target, max_seq_len, test_only))
            p.daemon = True
            example_generators.append(p)
